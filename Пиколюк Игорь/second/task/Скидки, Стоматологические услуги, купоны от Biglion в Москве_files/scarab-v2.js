var _scq = _scq || [];
_scq.push(['setMerchantId', '190E706B2CBE31F2']);
...
var ScarabUtil = function() {
  ...
  var makeDisableFunction = function(config) {
    var disabledDefs = config && config.disabledDefs || {};
    ...
    if (disabledType === "customFunction") {
      return function() {
        var fn = config && config.customDisabled || alwaysEnabled;
    ...
    if (disabledType === "urlDoesNotContain") {
      return function() {
        return ScarabArrays.filter(disabledPatterns, function(blockPath) {
          return window.location.href.indexOf(blockPath) !== -1;
        }).length > 0;
      };
    }
...
;var ScarabModule = function(win, config) {
  ...
  }, webpush:function(config) {
    config.merchantId = getMerchantId();
    ScarabUtil.loadModule(w, "emarsys_webpush", config);
  }, webpersonalization:function(config) {
    var hash = ScarabUtil.parseHashString(window);
    var moduleName = "sc_webpersonalization";
    if (hash[moduleName] !== undefined) {
      var val = hash[moduleName] || "{}";
      try {
        JSON.parse(val);
      ...
      sessionStorage.setItem(moduleName, val);
    }
    var moduleConfig = JSON.parse(sessionStorage.getItem(moduleName));
    var mergedConfig = ScarabUtil.merge({}, {merchantId:getMerchantId()}, config, moduleConfig);
    ScarabUtil.loadModule(w, "emarsys_webpersonalization", mergedConfig, "loader");
